...
export class AddMovieComponent implements OnInit {
  ...
  messages = {
    movieName: {
      requierd: 'The Name is required',
      exists: 'The Show name already exists, please choose another'
    },
    story: {
      requierd: 'The Story is required'
    },
    trailer: {
      requierd: 'The Trailer URL is required',
      valid: 'The Entered link is invalid'
    },
    catId: {
      requierd: 'The Category is required'
    },
    post: {
      requierd: 'The Poster is required'
    },
    actorId: {
      requierd: 'The Actor is required',
      exists: 'The Actor has already been added'
    },
    film: {
      requierd: 'The Show is required'
    },
    link: {
  ...
  AddMovie() {
    ...
    if (this.movieForm.valid && this.actorIds.length > 0 && !this.isMovieExists(movName)) {
      ...
      this.adminService.AddMovie(fd).subscribe(success => {
        this.sucMsg = 'The Show has been added Successfully';
        this.errMsg = '';
      }, ex => {
        console.log(ex);
        this.sucMsg = '';
  ...
  HandleFilmes(event: any) {
    if (event.target.files !== null && event.target.files.length > 0) {
       ...
       var id = $('#mov');
       (<any>id[0]).src = URL.createObjectURL(this.film);
       (<any>id.parent()[0]).load();
