...
namespace AngularToAPI.Repository.Admin
{
    public class AdminRepo : IAdminRepository
    {
        ...
        public async Task<Actor> AddActorAsync(string actorName, IFormFile img)
        {
            ...
            var filePath = Path.Combine(@"C:\Users\IT\Desktop\Cima\CinamaMovies\src\assets\images\actors", img.FileName);
            ...
            string catId, List<int> ids, string[] links)
        {
            ...
            var filePath = Path.Combine(@"C:\Users\IT\Desktop\Cima\CinamaMovies\src\assets\images\posts", img.FileName);
            using (FileStream fileStream = new FileStream(filePath, FileMode.Create))
            {
                await img.CopyToAsync(fileStream);
            }
            ...
            filePath = Path.Combine(@"C:\Users\IT\Desktop\Cima\CinamaMovies\src\assets\videos", video.FileName);
            using (FileStream fileStream = new FileStream(filePath, FileMode.Create))
            {
        ...
        public async Task<bool> EditMovieLinkAsync(MovieLink movieLink, IFormFile video)
        {
            ...
            if (video != null && video.Length > 0)
            {
                if (movLink.MovLink.ToLower() != video.FileName.ToLower())
                {
                    ...
                    var filePath = Path.Combine(@"C:\Users\IT\Desktop\Cima\CinamaMovies\src\assets\videos", video.FileName);
                    using (FileStream fileStream = new FileStream(filePath, FileMode.Create))
                    {
                        await video.CopyToAsync(fileStream);
                    }
        ...
        {
            var mov = await _db.MovieActors.FirstOrDefaultAsync(x => x.MovieId == movieActor.MovieId);
            if (mov == null)
                return false;
