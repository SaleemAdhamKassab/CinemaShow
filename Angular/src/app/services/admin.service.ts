...
export class AdminService {
  ...
  baseUrl = 'http://localhost:54504/api/Admin/';
  headers = {
  ...
  AddMovie(formData: FormData) {
    return this.http.post(this.baseUrl + 'AddMovie', formData,{ withCredentials: true }).pipe();
  }
  ...
  SearchMovies(search: any): Observable<Movie[]> {
  ...
  GetAllMovieLinks(search: any): Observable<MovieLink[]> {
  ...
  AddMovieLink(movieLink: MovieLink) {
    debugger
    return this.http.post(this.baseUrl + 'AddMovieLinkXXX', movieLink, this.headers).pipe();
  }
